...
(deftest set-works
  ...
  (testing "ttl works"
    ...
    (Thread/sleep 1500)
    (is (nil? (etcd/get! "etcd-clj/key")))))

(deftest get-works
  @(etcd/set "etcd-clj/key" "value")
  (is (= "value" (etcd/get! "etcd-clj/key")))
...
(deftest del-works
  ...
  (testing "deleting directories works"
    ...
    (is (etcd/get! "etcd-clj/a/b/c/d"))
    @(etcd/del "etcd-clj/a" :recursive true)
    (is (nil? (etcd/get! "etcd-clj/a/b/c/d")))))
...
(deftest CAD
  (let [mykey "etcd-clj/cad-key"
    ...
    (is (= "cad value" (-> mykey etcd/get deref :node :value)))
    (is (-> @(etcd/del mykey :prev-value "cad value")
            :errorCode
            nil?))
    (is (-> mykey etcd/get deref :errorCode (= 100)))))
